  ...
  module Screenshot
    class S3Saver
      ...
      def initialize(saver, s3_client, bucket_name, object_configuration, options={})
        ...
        @s3_host_base = options[:host_base]
      ...
      def save_and_upload_screenshot
        save_and do |type, local_file_path|
          File.open(local_file_path) do |file|
            ...
            s3_host_bucket = @s3_host_base ? "#{@s3_host_base}/#{bucket_name}" : "https://#{bucket_name}.s3-#{s3_region}.amazonaws.com"

            send("#{type}_path=", "#{s3_host_bucket}/#{s3_upload_path}")
          end
        end
      end
